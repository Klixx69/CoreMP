...
namespace SDK
{
	...
	public:
		...
		{
			bool bClass = name.starts_with("Class ");

			if (name.starts_with("Function ") || name.starts_with("Class "))
			{
				auto pos = name.rfind(' ');
				...
				std::string NameToFind = "/Script/" + name.substr(pos + 1);

				auto Object = FindObjectFast<T>(NameToFind);
				if (!Object)
					goto Slow;
				else
					return Object;
